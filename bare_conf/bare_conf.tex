...
\title{Using Deep Learning and EMG to predict non-audible speech}
...
Once the data is captured from the subjects, post-procesing of the data can begin. In order to remove the noise from the signals, filters are added after acquiring the data. We assumed that we are capturomg muslce movements below 4 Hz. We also want to eliminate the 60 Hz inteference from the surroundings. First, we appplied a low-pass filter centered at 4 Hz. The filters are ideal and they are designed around a sinc function. After applying a low pass filter, we add a high pass filter of 0.5 Hz, which removes the aliasing and the associated DC offset. The coinciding timestamps of the EMG data with the annotations are mapped together to create an input-ouput replationship, see Fig. \ref{fig: cleaned_signals}
